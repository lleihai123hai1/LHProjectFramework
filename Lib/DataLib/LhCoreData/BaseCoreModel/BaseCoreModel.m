        ...
        if(!skip){
            ...
            if (type) {
                ...
                if ([type isEqualToString:typeNumber] || [type isEqualToString:typeNumberFloat]) {
                    ...
                    value = [self stringFromDate:value key:@"yyyy-MM-dd HH:mm:ss"];
                    [values appendFormat:@"'%@',",value];
                }
...
#pragma mark date转换字符串
+(NSString *)stringFromDate:(NSDate *)date key:(NSString*)key{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setLocale:[NSLocale systemLocale]];
    [dateFormatter setDateFormat:key];
    NSString *destDateString = [dateFormatter stringFromDate:date];
    return destDateString;
    
}
#pragma mark 获取时间根据字符串
+(NSDate*)getDateFromString:(NSString*)dateStr key:(NSString*)key{
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init] ;
    [formatter setDateFormat:key];
